...
const AddTemplateTeamContainer = ({
  selectedTemplate,
  setIsAddTemplateTeamCon,
}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://margda.in:7000/api/margda.org/templates/add-teamplate-team",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-75 z-50">
      <div className="bg-white p-8 rounded-md shadow-2xl max-w-7xl">
        <h3 className="text-2xl font-semibold mb-8 text-gray-900 text-center">
          Add Team for {selectedTemplate.template}
        </h3>
        <form onSubmit={handleSubmit} className="space-y-8 min-h-[200px]">
          ...
          <div className="flex justify-end space-x-4 mt-8">
            <button
              type="button"
              onClick={() => setIsAddTemplateTeamCon(false)}
...
export default AddTemplateTeamContainer;
