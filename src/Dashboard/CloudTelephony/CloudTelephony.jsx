...
const CloudTelephony = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        "https://margda.in:7000/api/cloud_telephony/fetch_admin_calls",
        {
          method: "GET",
          headers: {
      ...
      if (data.Calls && Array.isArray(data.Calls)) {
        ...
        calls.sort((a, b) => new Date(b.call_start) - new Date(a.call_start));
        setCalls(calls);
        setAllCalls(calls);
      }
    } catch (err) {
  ...
  function filterByDateRange(data, startDate, endDate, searchQuery) {
    ...
    return data.filter((item) => {
      const itemTimestamp = new Date(item.call_start);
      return (
        itemTimestamp >= start &&
        itemTimestamp <= end &&
        ((item.caller &&
          ...
          <table className="w-full border-collapse bg-white">
            <thead>
              <tr className="bg-blue-500 text-white">
                ...
                <th className="py-3 px-4 text-left font-semibold">DID</th>
              </tr>
            </thead>
            <tbody>
