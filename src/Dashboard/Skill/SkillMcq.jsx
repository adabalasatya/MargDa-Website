...
import { FaBookOpen, FaBookReader, FaSearch, FaChevronLeft, FaChevronRight } from 'react-icons/fa';
...
const SkillMCQ = () => {
  ...
  const skillsOptions = [
    { value: '1', label: 'Career Counselling (26)' },
    { value: '2', label: 'HR Management (31)' },
    { value: '3', label: 'Digital & Social Media (0)' },
    { value: '4', label: 'Web Design - HTML (24)' },
    { value: '5', label: 'Business Communication (12)' },
    { value: '6', label: 'Inside Sales (36)' },
    { value: '7', label: 'teaching (0)' },
    { value: '8', label: 'ha (0)' },
    { value: '9', label: 'Soft Skills (0)' },
    { value: '10', label: 'Management (0)' },
    { value: '11', label: 'Others (0)' },
  ];
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(mcqs.length / recordsPerPage)) setCurrentPage(currentPage + 1);
  };
  ...
  const filteredData = mcqs.filter(mcq => 
  ...
  const filteredMcqs = filteredData.slice(indexOfFirstRecord, indexOfLastRecord);
  ...
  useEffect(() => {
    const handleDropdownPosition = () => {
      ...
      if (selectElement) {
        ...
        if (editorElement && selectElement.getBoundingClientRect().bottom > editorElement.getBoundingClientRect().top) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-lg shadow-md">
        <div className="mb-4">
          <label className="block text-sm font-bold mb-2 flex items-center">
            <FaBookReader className="inline-block text-blue-500 mr-2" /> Select Skill
          </label>
          <Select
            value={skill}
            onChange={setSkill}
        ...
        <div className="mb-4">
          ...
          <Editor
            ...
            onEditorChange={(content, editor) => handleEditorChange(content, editor, 'question')}
          />
          <span id="ques_err" className="text-red-500 hidden">* Minimum Length 2000 characters</span>
        </div>
        {['opt1', 'opt2', 'opt3', 'opt4'].map((opt, index) => (
          <div key={opt} className="mb-4">
            ...
            <Editor
              ...
              onEditorChange={(content, editor) => handleEditorChange(content, editor, index)}
            />
            <span id={`${opt}_err`} className="text-red-500 hidden">* Minimum Length 2000 characters</span>
          </div>
        ))}
        <div className="mb-4">
          ...
          <Editor
            ...
            onEditorChange={(content, editor) => handleEditorChange(content, editor, 'explanation')}
          />
          <span id="explain_err" className="text-red-500 hidden">* Minimum Length 2000 characters</span>
        </div>
        <div className="mb-4">
          ...
          <div className="flex space-x-4">
            {['E', 'N', 'D', 'T'].map((val) => (
              <label key={val} className="inline-flex items-center">
                ...
                />
                <span className="ml-2">{val === 'E' ? 'Easy' : val === 'N' ? 'Normal' : val === 'D' ? 'Difficult' : 'Very Tough'}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-300">Submit</button>
      </form>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Option 1</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Option 2</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Option 3</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Option 4</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Explain</th>
...
<div className="flex items-center justify-between mt-6 bg-white p-4 rounded-lg shadow-md">
  ...
  <div className="flex items-center space-x-2">
    ...
    <button
      ...
      className={`px-4 py-2 bg-gray-200 text-gray-700 rounded transition duration-300 ${currentPage === 1 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-300"}`}
    >
    ...
    {getPaginationRange().map((page) => (
      <button
        ...
        className={`px-4 py-2 ${currentPage === page ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"} rounded transition duration-300`}
      >
        {page}
      </button>
    ))}
...
<span className="px-4 py-2 font-semibold text-white bg-blue-500 rounded shadow-md">
   {currentPage}
    ...
    <button
      ...
      className={`px-4 py-2 bg-gray-200 text-gray-700 rounded transition duration-300 ${currentPage === Math.ceil(filteredData.length / recordsPerPage) ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-300"}`}
    >
