...
const AllAdvisors = () => {
  ...
  const [searchAdvisorsQuery, setSearchAdvisorsQuery] = useState("");
  ...
  const fetchAdvisors = async () => {
    try {
      const response = await fetch(
        "https://margda.in:7000/api/admin/advisors/get-all-advisors",
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (response.ok) {
        setConsultants(result.Advisors);
        setAllConsultants(result.Advisors);
      } else {
        throw new Error("Invalid data format received from the API");
      }
    } catch (error) {
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    console.log(editFormData.advisorID);
  ...
  useEffect(() => {
    if (allConsultants && allConsultants.length > 0) {
      const filter = allConsultants.filter(
        (item) =>
          item.details.toLowerCase().includes(searchAdvisorsQuery) ||
          item.avatar_name.toLowerCase().includes(searchAdvisorsQuery) ||
          item.heading.toLowerCase().includes(searchAdvisorsQuery)
      );
      setConsultants(filter);
    }
  }, [searchAdvisorsQuery]);

  return (
    <div className="p-8 bg-gray-50">
      <h2 className="text-3xl font-bold text-gray-800 mb-8">All Advisors</h2>
      <div className="relative mb-3">
        <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
        <input
          type="text"
          value={searchAdvisorsQuery}
          onChange={(e) => setSearchAdvisorsQuery(e.target.value)}
          className="border border-gray-300 p-2 pl-8 rounded w-[40%]"
          placeholder="Search"
        />
      </div>
