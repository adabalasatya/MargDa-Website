...
const dummySkills = [
    { id: 35, skill: 'Writing' },
    { id: 34, skill: 'Troubleshooting' },
    { id: 33, skill: 'Time Management' },
    { id: 32, skill: 'Technology Design' },
    { id: 31, skill: 'Systems Evaluation' },
    { id: 30, skill: 'Systems Analysis' },
    { id: 29, skill: 'Speaking' },
    { id: 28, skill: 'Social Perceptiveness' },
    { id: 27, skill: 'Service Orientation' },
    { id: 26, skill: 'Scientific' },
];
...
const CareerSkills = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!skill.trim()) newErrors.skill = true;
        ...
        const newSkill = {
            id: skills.length + 26, // Continue from the last ID in dummy data
            skill: skill.trim(),
        };
    ...
    const handleDelete = (id) => {
        if (window.confirm(`Are you sure you want to delete skill ID: ${id}?`)) {
    ...
    const filteredSkills = skills.filter(
        (sk) =>
            sk.id.toString().includes(searchTerm) ||
            sk.skill.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const indexOfLastRecord = currentPage * recordsPerPage;
    const indexOfFirstRecord = indexOfLastRecord - recordsPerPage;
    const currentRecords = filteredSkills.slice(indexOfFirstRecord, indexOfLastRecord);
    const totalPages = Math.ceil(filteredSkills.length / recordsPerPage);
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto bg-white shadow-md rounded-lg border border-blue-300">
            <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center">
                <FaChalkboardTeacher className="mr-2 text-blue-500" /> Career-Skills
            </h2>
            ...
            <form onSubmit={handleSubmit} className="mb-8 space-y-4 p-4 rounded-lg ">
                <div className="relative">
                    ...
                    {errors.skill && <span className="text-red-500 text-sm">* Skill is required</span>}
                </div>
                <button
                    type="submit"
                    ...
                    <tbody>
                        {currentRecords.map((sk) => (
                            <tr key={sk.id} className="hover:bg-gray-50 transition">
                                ...
                                <td className="border border-gray-200 px-4 py-2 text-black-800">{sk.skill}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <div className="flex justify-between items-center mt-6 text-black-600">
                <span>Showing {indexOfFirstRecord + 1} to {Math.min(indexOfLastRecord, filteredSkills.length)} of {filteredSkills.length} entries</span>
